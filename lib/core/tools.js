...
// 加密
module.exports.gAesEncrypt = function (src, key) {
    return cryptojs.AES.encrypt(src, key).toString();
}

// 解密
module.exports.gAesDecrypt = function (sign, key) {
    return cryptojs.AES.decrypt(sign, key).toString(cryptojs.enc.Utf8);
}
...
module.exports.makeMap = function(table, key, value) {
    if (!table) {
        table = {};
    }
...
module.exports.makeLastWeekDateStr = function (timeNow) {
    let today = new Date(timeNow.valueOf());
    today.setHours(0);
    today.setMinutes(0);
    today.setSeconds(0);
    let lastWeekDay = new Date(today.valueOf() - 7*24*3600*1000);
    let weekDays = 7;
    let dateStrs = [];
    for (let index = 0; index < weekDays; index++) {
        let diffDay = index - lastWeekDay.getDay();
        dateStrs.push(moment(lastWeekDay.valueOf() + diffDay*24*3600*1000).format("YYYYMMDD"));
    }
    return dateStrs;
}
